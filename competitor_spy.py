...
Or deploy as a Railway cron job (recommended)
"""
...
def init_instagram() -> Optional["IGClient"]:
    ...
    try:
        ...
        log.warning("ChallengeRequired: Instagram blocked login from this IP. Skipping scrape this cycle.")
        return None
...
def _map_media_type(media_type: int) -> str:
    return {1: "single_image", 2: "carousel", 8: "carousel"}.get(media_type, "reel")


CLASSIFIER_PROMPT = """You are a content classifier for personal finance Instagram posts.

Given a list of post captions, classify each into one of:
investing basics, stock market, crypto, budgeting hacks, debt payoff, side hustle,
money mindset, real estate, retirement / FIRE, tax tips, market news, financial mistake / story, other

Return ONLY a JSON array: [{"index": 0, "topic": "string", "confidence": "high"|"medium"|"low"}, ...]
...
def classify_posts(posts: list[dict], claude: anthropic.Anthropic) -> list[dict]:
    if not posts:
        return posts
    captions = [{"index": i, "caption": p.get("caption_preview", "")[:200]} for i, p in enumerate(posts)]
    try:
        response = claude.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=2000,
            messages=[{"role": "user", "content": CLASSIFIER_PROMPT + json.dumps(captions)}]
        )
...
Scraped competitor data (last 7 days, sorted by likes):
{posts_json}
...
{{
  ...
  "content_gaps": [string],
  "top_posts_analysis": [{{"handle": string, "topic": string, "why_it_worked": string, "angle_to_steal": string}}],
  "avoid": [string],
...
def analyse_trends(posts: list[dict], claude: anthropic.Anthropic) -> Optional[dict]:
    ...
    recent = [p for p in posts if p.get("posted_at") and datetime.fromisoformat(p["posted_at"].replace("Z", "")) > cutoff]
    ...
    try:
        response = claude.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=2000,
            messages=[{"role": "user", "content": TREND_PROMPT.format(posts_json=json.dumps(top_posts, default=str, indent=2))}]
        )
        raw = response.content[0].text.strip()
        if raw.startswith("```"):
            raw = raw.split("```")[1]
...
def save_posts_to_supabase(posts: list[dict], db: Client) -> int:
    ...
    for post in posts:
        try:
            db.table("competitor_posts").upsert(post, on_conflict="post_url").execute()
            saved += 1
        except Exception as e:
...
def save_trends_to_supabase(trends: dict, db: Client):
    ...
    try:
        ...
        if existing.data:
            db.table("agent_memory").update(payload).eq("date", today).execute()
        else:
            db.table("agent_memory").insert({"date": today, **payload}).execute()
...
def _mock_posts() -> list[dict]:
    return [
        {"handle": "test_account_1", "post_url": "https://www.instagram.com/p/mock1/",
         "caption_preview": "The S&P 500 has made millionaires out of people who just didn't touch it.",
         "likes": 2847, "comments": 134, "post_type": "carousel",
         "posted_at": (datetime.utcnow() - timedelta(days=1)).isoformat(),
         "scraped_at": datetime.utcnow().isoformat(), "topic_detected": None},
        {"handle": "test_account_2", "post_url": "https://www.instagram.com/p/mock2/",
         "caption_preview": "I paid off $34,000 in debt in 18 months on a $52k salary.",
         "likes": 5201, "comments": 389, "post_type": "carousel",
         "posted_at": (datetime.utcnow() - timedelta(days=2)).isoformat(),
         "scraped_at": datetime.utcnow().isoformat(), "topic_detected": None},
        {"handle": "test_account_1", "post_url": "https://www.instagram.com/p/mock3/",
         "caption_preview": "Your emergency fund is costing you money if it's in a regular savings account.",
         "likes": 1923, "comments": 201, "post_type": "reel",
...
if __name__ == "__main__":
    ...
    if args.test:
        def init_instagram(): return None
